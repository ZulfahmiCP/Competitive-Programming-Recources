...
int main(){
    ...
    FastIO();
    int t,n,q,mx(1e3); cin >> t;
    while(t--){
        cin >> n >> q;
        vector<vector<ll>> A(mx + 1, vector<ll>(mx + 1, 0));
        vector<vector<ll>> cnt(mx + 1, vector<ll>(mx + 1, 0));
        ...
        for(int i = 1; i <= mx; i++)
            for(int j = 1; j <= mx; j++)
                A[i][j] = A[i - 1][j] + A[i][j - 1] - A[i - 1][j - 1] + cnt[i][j];
        
        for(int i = 0, a,b,c,d; i < q; i++){
            cin >> a >> b >> c >> d;
            cout << A[a][b] + A[c - 1][d - 1] - A[c - 1][b] - A[a][d - 1] << '\n';
        }
    }
