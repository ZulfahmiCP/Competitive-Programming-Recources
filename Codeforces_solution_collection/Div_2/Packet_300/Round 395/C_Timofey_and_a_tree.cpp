...
int main(){
    ...
    FastIO();
    int n,same_color,cur_color,ans; cin >> n;
    ...
    function<void(int, int)> explore = [&](int node, int parent) -> void {
        same_color &= (color[node] == cur_color);
        for(int &adjNode : adjList[node])
            if(adjNode != parent)
                explore(adjNode, node);
    };

    function<bool(int)> solve = [&](int node) -> bool {
        ans = 1;

        for(int &adjNode : adjList[node]){
            cur_color = color[adjNode];
            same_color = 1;
            explore(adjNode, node);
            ans &= same_color;
        }
    ...
    bool ansA = solve(rootA++), ansB = solve(rootB++);
