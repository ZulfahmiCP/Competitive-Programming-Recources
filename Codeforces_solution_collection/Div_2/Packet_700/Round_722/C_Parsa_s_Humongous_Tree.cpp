    ...
    while(t--){
        ...
        function<void(int, int)> explore = [&](int node, int parent) -> void {
            for(int adjNode : adjList[node]) if(adjNode != parent) {
                ...
                dp[node][0] += max(abs(L[node] - L[adjNode]) + dp[adjNode][0], abs(L[node] - R[adjNode]) + dp[adjNode][1]);
                dp[node][1] += max(abs(R[node] - L[adjNode]) + dp[adjNode][0], abs(R[node] - R[adjNode]) + dp[adjNode][1]);
            }
        };
