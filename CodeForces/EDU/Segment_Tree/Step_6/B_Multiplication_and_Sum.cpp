...
struct segTree
{
    ...
    void merge(segNode &cur,segNode &left,segNode &right)//change this function
    {
        ...
        cur.val = (left.val + right.val)%prime;
    }
    void propogate(int id)//change this function
    {
        ...
        seg[id].val=(seg[id].val*lazy[id].val)%prime;
        if(l+1!=r)
        {
            int mid=(l+r)>>1LL,left=2*id+1,right=2*id+2;
            lazy[left].val=(lazy[id].val*lazy[left].val)%prime;
            lazy[right].val=(lazy[id].val*lazy[right].val)%prime;
        }
        lazy[id].val=1;
    }
