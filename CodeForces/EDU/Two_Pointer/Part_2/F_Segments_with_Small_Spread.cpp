...
template <typename T>
    void debug(const multiset<T>& container);
template <typename T>
...
int main(){
    ...
    for(int i = 0, j = 0; j < n; j++){
        ...
        while(*smax.begin() - *smin.begin() > k){
            smin.erase(smin.find(A[i]));
            smax.erase(smax.find(A[i]));
            i++;
        }
...
template <typename T> void debug(const multiset<T>& container) { cerr << '['; bool comma = 0; for (const auto& st : container) { if (comma) cerr << ", "; printDbg(st); comma = 1; } cerr << "]\n";}
