...
int main(){
    ...
    for(int i = 0, diameter,vertice; i < m; i++){
        cin >> n;
        ...
        function<void(int, int, int)> explore = [&](int node, int parent, int depth) -> void {
            if(depth > diameter)
                diameter = depth, vertice = node;
            
            for(int &adjNode : adjList[node])
                if(adjNode != parent)
                    explore(adjNode, node, depth + 1);
        };
