...
int main(){
    ...
    while(t--){
        ...
        vector<bool> visited(n, 0), is_cycles(n, 0);
        stack<int> cycle;
        ...
        function<void(int, int)> find_cycle = [&](int node, int parent) -> void {
            ...
            for(int &adjNode : adjList[node])
                if(visited[adjNode]){
                    if(adjNode == parent || found)
                        continue;

                    while(!cycle.empty() && cycle.top() != adjNode){
                        is_cycles[cycle.top()] = 1;
                        cycle.pop();
                    }
        ...
        function<void(int, int)> explore = [&](int node, int parent) -> void {
            ...
            for(int &adjNode : adjList[node])
                if(adjNode != parent && !is_cycles[adjNode])
        ...
        for(int i = 0; i < n; i++)
            if(is_cycles[i]){
                group = 0;
                explore(i, -1);
                ans += 1LL * group * (2 * (n - group) + (group - 1));
            }
