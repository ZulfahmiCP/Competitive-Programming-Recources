...
int main(){
    ...
    for(int i = 1; i < 3; i++){
        ...
        for(int j = 0; j < n; j++)
            if(depth[j] > depth[ans[i]])
                ans[i] = j, mx = depth[j];
    }

    vector<bool> in_path(n, 0);

    function<void(int, int)> find_path = [&](int node, int parent) -> void {
        for(int &adjNode : adjList[node]){
            if(adjNode == parent)
                continue;
            find_path(adjNode, node);
            if(in_path[adjNode])
                in_path[node] = 1;
        }
    };

    in_path[ans[1]] = in_path[ans[2]] = 1;
    find_path(ans[2], -1);

    for(int i = 0; i < n; i++){
        if(!in_path[i] || i == ans[1] || i == ans[2])
            continue;
    ...
    for(int i = 0; i < n; i++)
        if(!in_path[i] && depth[i] > depth[ans[0]])
            ans[0] = i;
