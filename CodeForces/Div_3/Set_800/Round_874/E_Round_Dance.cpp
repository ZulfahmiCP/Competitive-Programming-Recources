...
int main(){
    ...
    FastIO();
    int t,n,min_round,max_round,extra,sum; cin >> t;
    while(t--){
        cin >> n;
        ...
        function<bool(int, int)> cycle = [&](int node, int parent) -> bool {
            bool check = 0;
            visited[node] = -1;
            sum++;
            ...
            for(int adjNode : adjList[node]) if(adjNode != parent) {
                if(visited[adjNode] == -1)
                    check = 1;
                else if(visited[adjNode] == 0 && cycle(adjNode, node))
                    check = 1;
            }
