...
vector<pair<int, int>> solve1(int n, vector<int> A) {
...
int getIndex(int a, int b) {
    if(a > b) swap(a, b);
    return (a == 1 ? (b == 2 ? 1 : 3) : 2);
}

vector<pair<int, int>> solve2(int n, vector<int> A) {
    vector<pair<int, int>> ans;
    int sum = accumulate(all(A), 0);
    ...
    int cur = 4, prev;
    for(int i = 1; i < 3; i++){
        int index = getIndex(line[i], line[0]) - 1;
        prev = line[i];
