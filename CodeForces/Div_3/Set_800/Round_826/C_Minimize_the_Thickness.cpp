...
int main(){
    ...
    while(t--){
        ...
        auto thickness = [&](int x) -> int {
            int cur_sum(0), min_leng(0), start(0);
            for(int i = 0; i < n; i++){
                cur_sum += A[i];
                ...
                if(cur_sum == x){
                    cur_sum = 0;
                    min_leng = max(min_leng, i - start + 1);
                    start = i + 1;
                }
            }
 
            return (cur_sum ? n : min_leng);
        };
 
        sum = accumulate(all(A), 0), bound = *max_element(all(A)), res = n;
        for(int i = 1; i*i <= sum; i++){
            if(sum % i)
                continue;
            if(i >= bound)
                res = min(res, thickness(i));
            if(sum/i >= bound)
                res = min(res, thickness(sum/i));
        }
