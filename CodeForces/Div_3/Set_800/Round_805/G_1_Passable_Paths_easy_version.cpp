...
int main(){
    ...
    function<void(int, int)> find_depth = [&](int node, int parent) -> void {
        if(node) depth[node] = depth[parent] + 1;
        
        for(int &adjNode : adjList[node])
            if(adjNode != parent)
                find_depth(adjNode, node);
    };

    find_depth(0, -1);

    int q, m; cin >> q;
    vector<bool> choosen(n, 0);

    function<bool(int, int, int)> exist = [&](int node, int parent, int cnt) -> bool {
        if(choosen[node])
            cnt++;
        if(cnt == m)
            return 1;
        ...
        for(int &adjNode : adjList[node])   
            if(adjNode != parent && exist(adjNode, node, cnt))
                return 1;

        return 0;
    };  

    for(int i = 0, start; i < q; i++){
        cin >> m;
        fill(all(choosen), 0);
        ...
        for(int j = 0, v; j < m; j++){
            cin >> v, v--;
            choosen[v] = 1;
            if(depth[v] > depth[start])
                start = v;
        }
        
        cout << (exist(start, -1, 0) ? "YES" : "NO") << '\n';
    }
