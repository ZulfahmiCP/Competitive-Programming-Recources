...
struct mod_int {
    ...
    friend mod_int power(mod_int a, ll b){if(b<0)return 0;mod_int c(1);for(;b > 0;a *= a,b >>= 1)if(b & 1)c *= a;return c;}
...
int main(){
    ...
    while(t--){
        ...
        zero = power(two, n) - power(two, n - c);
        a = power(two, a);

        for(int i = 1; i <= n; i += 2)
            neg += a * C.comb(b.val, i);

        pos = power(two, n) - neg - zero - 1;
