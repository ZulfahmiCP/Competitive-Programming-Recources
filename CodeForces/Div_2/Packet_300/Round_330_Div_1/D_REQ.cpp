...
struct MO {
    ...
    vector<int> val, occ, res;
    vector<mint> pref, phi, phi_inv;
    vector<query> queries;
    Factor Fac;

    MO(int n) : N(n), Q(0), ans(1), arr(N), val(N), occ(MX, 0), 
                Fac(MX), pref(N + 1), phi(MX), phi_inv(MX) {}
