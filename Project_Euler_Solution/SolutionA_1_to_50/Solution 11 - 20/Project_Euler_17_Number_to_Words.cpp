...
const vector<string> MAGNITUDES = {"", "Thousand", "Million", "Billion"};

string numToWords(ll n) {
    if(!n) return "Zero";
    if(n == 1e12) return "One Trillion";

    string result = "", words;

    for(int i = 0, x; i < MAGNITUDES.size() && n; i++){
        words = "";
        x = n % 1000, n /= 1000;
        ...
        if(x/10 == 1){
            words += (words.empty() ? "" : " ") +  TEENS[x % 10];
            x = 0;
        }

        if(x > 10)
            words += (words.empty() ? "" : " ") + TENS[x/10];
        x %= 10;
        
        if(x > 0)
            words += (words.empty() ? "" : " ") + ONES[x];

        result = words + (i ? " " + MAGNITUDES[i] : "") + (result.empty() ? "" : " " + result);
    }
