...
int main(){
    ...
    FastIO();
    int n(20),k(4),largest_product = 0; 
    ...
    function<int(pair<int, int>, pair<int, int>, int)> 
    product = [&](pair<int, int> cur, pair<int, int> dir, int count) -> int {
        if(cur.X < 0 || cur.Y < 0 || cur.X == n || cur.Y == n)
            return 0;
        if(count == k)
            return grid[cur.X][cur.Y];
        
        return grid[cur.X][cur.Y] * product({cur.X + dir.X, cur.Y + dir.Y}, dir, count + 1);
    };  
    ...
    for(int i = 0; i < n; i++)
        for(int j = 0; j < n; j++)
            for(auto dir : direction)
                largest_product = max(largest_product, product({i, j}, dir, 1));
