...
int main(){
    ...
    function<void(int, int, int)> build = [&](int pos, int l, int r) -> void {
        ...
        min_val[pos] = min(min_val[2*pos + 1], min_val[2*pos + 2]);
    };

    function<int(int, int, int, int, int)> get_min = [&](int pos, int l, int r, int L, int R) -> int {
        if(l > R || r < L)
            return mx;
        ...
        int m = (l + r)/2;
        return min(get_min(2*pos + 1, l, m, L, R), get_min(2*pos + 2, m + 1, r, L, R));
    }; 
