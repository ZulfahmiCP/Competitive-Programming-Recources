...
struct Binomial {
    ...
    int perm(int A, int B) {
        if(A < B) return 0;
        return (fact[A] * inv[A - B]).val;
    }

    int perm_rep(int A, const vector<int> &B) {
        mint res = fact[A];
        for(const int &b : B)
            res *= inv[b];
        return res.val;
    ...
    int comb_rep(int A, int B) {
        return comb(A + B - 1, B).val;
    }
...
int main(){
    ...
    Binomial C(n);
    cout << C.comb(n - a - b, n / 2 - a) - C.comb(n - a - b, n / 2 - a - 1) << '\n';
