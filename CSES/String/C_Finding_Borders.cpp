...
struct String {
    ...
    String(const string &s, int k = 25) : N(sz(s)), K(k), S(s), hash(N + 1), power(N + 1) {
        power[0] = 1;
        for(int i = 1; i <= N; i++)
            power[i] = power[i - 1] * K;
        for(int i = 0; i < N; i++)
            hash[i + 1] = hash[i] * K + (s[i] - 'a' + 1);
    }

    // return i ... j - 1
    int sub_hash(int i, int j) {
        return (hash[j] - hash[i] * power[j - i]).val;
    }
...
int main(){
    ...
    for(int i = 1; i < n; i++)
        if(S.pref_hash(i) == S.suff_hash(n - i))
            cout << i << ' ';
